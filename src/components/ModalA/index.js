...
import { API_URL, BEARER_TOKEN, COMPANY_ID } from "../../config/constants";
...
const ModalA = (props) => {
  ...
  const fetchAllContacts = async (reload = false, q = null) => {
    try {
      ...
      let params = { companyId: COMPANY_ID, page: reload ? 1 : page };
  ...
  return (
    ...
    >
      <Modal.Header>
        <div>
          <Button className="mr-4 cursor-pointer btn-a">All Contacts</Button>
          <Button onClick={props.switchModal} className="cursor-pointer btn-b">
            US Contacts
          </Button>
        </div>
...
export default withRouter(ModalA);
